...
// the Vercel env to enable it. We take the median decimal price per team
// across books, de-vig to a fair implied probability, and cache the result
// in the kv store for TTL_MS (6 hours), so the upstream is only hit ~4 times
// per day — well within The Odds API's free tier of 500 req/month.
//
...
export const GET = async (req) => {
  ...
  const cached = await kvGet("odds-api");
